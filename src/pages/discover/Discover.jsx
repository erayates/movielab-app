    ...
    fetchData('/discover/movie').then(response => {
      response.results.map((item, idx) => {
        ...
        pages.push(({ style }) => <animated.div key={idx}
          className='relative rounded-2xl ml-20 w-full lg:ml-[24rem]'
          style={{ ...style }}>
          <div className='w-full h-full rounded-2xl relative bg-no-repeat bg-center bg-cover' style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original${item.backdrop_path})`, backgroundColor: 'rgba(0,0,0,0.8)', backgroundBlendMode: 'darken', }}>
            <div className=' h-full w-full md:w-[600px]  bg-black/50 flex  items-center rounded-2xl' >
              <div className='md:w-[400px] p-6 md:p-12'>
                ...
                <button className='bg-red-600 text-white py-2 px-6 mt-3 rounded-lg shadow-lg shadow-red-600 block text-[11px] md:text-[16px] animate-pulse'>
                  <BtnPlayIcon/>
                  Watch The Trailer
                </button>
              </div>
