...
function SearchResult() {
  ...
  useEffect(() => {
    fetchData(`/search/multi?query=${query}&include_adult=false&language=en-US&page=${page}`)
      .then((res) => {
      ...
      <div className='movies-container flex flex-wrap w-full justify-center'>
        {
          ...
          searchResult.map((item) => {
            ...
            return (
              <div className=' w-[150px] h-[250px]  md:w-[200px] md:h-[350px] bg-transparent rounded-t-2xl my-5 mr-[1.99rem] relative cursor-pointer' key={item.id} onClick={() => navigate(`/${item.media_type}/${item.id}`)}>
                ...
                <div className='w-full h-1/4 flex flex-col pt-3 bg-[#151515] p-4 rounded-b-2xl'>
                  <h3 className='text-white text-[14px] font-semibold'>{item.media_type === 'movie' ? item.title : item.name}</h3>
