...
const Movies = () => {
    ...
    return (
        <section className='movies ml-96 p-16 pt-8'>
            <h1 className="movies-title text-[64px] text-transparent bg-clip-text bg-gradient-to-r font-bold from-white to-blue-500">Latest Movies</h1>
            <p className='movies-subtitle mt-[-20px] mb-6 font-normal text-white/50 text-[14px]'>Check out all the movies released in the last 14 days!</p>
            <div className='movies-container flex flex-wrap w-full justify-center'>
            {pageData.length > 0 &&
                pageData.map((item) => {
                    const movieGenreList = getMovieGenres(item);
                    return(
                    <div className=' w-[150px] h-[250px]  md:w-[200px] md:h-[350px] bg-transparent rounded-t-2xl my-5 mr-[1.99rem] relative cursor-pointer' key={item.id} onClick={() => navigate(`/movie/${item.id}`)}>
